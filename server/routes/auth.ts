...
router.post('/register', async (req, res) => {
  try {
    ...
    const existingProfile = await db.prepare('SELECT id FROM profiles WHERE username = ?').get(username);
    if (existingProfile) return res.status(400).json({ error: 'Username already taken' });

    const id = uuid();
    const passwordHash = await bcrypt.hash(password, 10);
    await db.prepare('INSERT INTO users (id, email, password_hash) VALUES (?, ?, ?)').run(id, email, passwordHash);
    ...
    await db.prepare('INSERT INTO refresh_tokens (id, user_id, token_hash, expires_at) VALUES (?, ?, ?, ?)').run(uuid(), id, refreshTokenHash, refreshExpiresAt);

    const ip = String(req.headers['x-forwarded-for'] || req.socket.remoteAddress || '').split(',')[0].trim();
    const userAgent = String(req.headers['user-agent'] || '');
    try {
      await db.prepare('INSERT INTO login_history (id, user_id, ip, user_agent, success) VALUES (?, ?, ?, ?, 1)').run(uuid(), id, ip, userAgent);
      await db.prepare('INSERT INTO sessions (id, user_id, token_hash, ip, user_agent, expires_at) VALUES (?, ?, ?, ?, ?, ?)').run(uuid(), id, refreshTokenHash, ip, userAgent, refreshExpiresAt);
    } catch {}
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!user) {
      const ip = String(req.headers['x-forwarded-for'] || req.socket.remoteAddress || '').split(',')[0].trim();
      try {
        await db.prepare('INSERT INTO login_history (id, user_id, ip, user_agent, success) VALUES (?, ?, ?, ?, 0)').run(
    ...
    try {
      await db.prepare('INSERT INTO refresh_tokens (id, user_id, token_hash, expires_at) VALUES (?, ?, ?, ?)').run(
        uuid(), user.id, refreshTokenHash, refreshExpiresAt
      );
      await db.prepare('INSERT INTO login_history (id, user_id, ip, user_agent, success) VALUES (?, ?, ?, ?, 1)').run(
        uuid(), user.id, ip, userAgent
      );
      await db.prepare('INSERT INTO sessions (id, user_id, token_hash, ip, user_agent, expires_at) VALUES (?, ?, ?, ?, ?, ?)').run(
...
router.post('/logout', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    if (refreshToken) {
      const tokenHash = hashToken(refreshToken);
      await db.prepare('UPDATE refresh_tokens SET revoked = 1 WHERE token_hash = ? AND user_id = ?').run(tokenHash, req.userId);
    }
...
router.post('/reset-password', async (req, res) => {
  try {
    ...
    await db.prepare('INSERT INTO password_reset_tokens (id, user_id, token, expires_at) VALUES (?, ?, ?, ?)').run(uuid(), user.id, token, expiresAt);
...
router.post('/reset-password-token', async (req, res) => {
  try {
    ...
    const resetToken = await db.prepare('SELECT * FROM password_reset_tokens WHERE token = ? AND used = 0').get(token) as any;
    ...
    await db.prepare('UPDATE users SET password_hash = ? WHERE id = ?').run(hash, resetToken.user_id);
    await db.prepare('UPDATE password_reset_tokens SET used = 1 WHERE id = ?').run(resetToken.id);

    await db.prepare('UPDATE sessions SET is_active = 0 WHERE user_id = ?').run(resetToken.user_id);
