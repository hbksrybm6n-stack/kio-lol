...
router.get('/profile/:username', apiKeyAuth, (req: any, res) => {
  try {
...
router.get('/profile/:username/links', apiKeyAuth, (req: any, res) => {
  try {
    let profile = db.prepare('SELECT id FROM profiles WHERE username = ? AND is_active = 1').get(req.params.username) as any;
    if (!profile) {
      profile = db.prepare('SELECT id FROM profiles WHERE slug = ? AND slug != \'\' AND is_active = 1').get(req.params.username) as any;
    }
    if (!profile) return res.status(404).json({ error: 'Profile not found' });

    const links = db.prepare("SELECT id, title, url, description, icon, color, background_color, hover_color, click_count, sort_order FROM links WHERE profile_id = ? AND is_active = 1 AND visibility = 'public' ORDER BY sort_order ASC").all(profile.id);
    res.json(links);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
router.get('/profile/:username/socials', apiKeyAuth, (req: any, res) => {
  try {
    let profile = db.prepare('SELECT id FROM profiles WHERE username = ? AND is_active = 1').get(req.params.username) as any;
    if (!profile) {
      profile = db.prepare('SELECT id FROM profiles WHERE slug = ? AND slug != \'\' AND is_active = 1').get(req.params.username) as any;
