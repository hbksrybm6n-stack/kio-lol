...
router.get('/', (_req, res) => {
  res.json(db.prepare('SELECT * FROM badges ORDER BY name').all());
});
...
router.post('/assign', authMiddleware, (req: AuthRequest, res) => {
  ...
  try {
    db.prepare('INSERT OR IGNORE INTO user_badges (id, profile_id, badge_id, awarded_by) VALUES (?, ?, ?, ?)').run(uuid(), profile_id, badge_id, req.userId);
    res.json({ ok: true });
  } catch (err: any) {
...
router.delete('/remove', authMiddleware, (req: AuthRequest, res) => {
  const admin = db.prepare('SELECT is_admin FROM profiles WHERE user_id = ?').get(req.userId!) as any;
  if (!admin?.is_admin) return res.status(403).json({ error: 'Forbidden' });
  const { profile_id, badge_id } = req.body;
  if (!profile_id || !badge_id) return res.status(400).json({ error: 'profile_id and badge_id required' });
  db.prepare('DELETE FROM user_badges WHERE profile_id = ? AND badge_id = ?').run(profile_id, badge_id);
  res.json({ ok: true });
});
