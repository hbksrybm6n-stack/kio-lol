...
router.put('/email', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const existing = await db.prepare('SELECT id FROM users WHERE email = ? AND id != ?').get(newEmail, req.userId);
    if (existing) return res.status(400).json({ error: 'Email already in use' });

    await db.prepare('UPDATE users SET email = ? WHERE id = ?').run(newEmail, req.userId);
    ...
    await db.prepare('INSERT INTO email_verifications (id, user_id, email, token, expires_at) VALUES (?, ?, ?, ?, ?)').run(
...
router.put('/password', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const userData = await db.prepare('SELECT email FROM users WHERE id = ?').get(req.userId) as any;
    if (userData?.email) {
...
router.get('/login-history', authMiddleware, async (req: AuthRequest, res) => {
  try {
...
router.delete('/sessions/:id', authMiddleware, async (req: AuthRequest, res) => {
  try {
    await db.prepare('UPDATE sessions SET is_active = 0 WHERE id = ? AND user_id = ?').run(req.params.id, req.userId);
    res.json({ ok: true });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
router.post('/sessions/revoke-all', authMiddleware, async (req: AuthRequest, res) => {
  try {
    await db.prepare('UPDATE sessions SET is_active = 0 WHERE user_id = ?').run(req.userId);
    res.json({ ok: true });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
router.post('/email/verify', authMiddleware, async (req: AuthRequest, res) => {
  try {
...
router.post('/email/verify/confirm', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    await db.prepare('UPDATE email_verifications SET verified = 1 WHERE id = ?').run(verification.id);
...
router.post('/2fa/enable', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const existing = await db.prepare('SELECT * FROM totp_secrets WHERE user_id = ? AND enabled = 1').get(req.userId!) as any;
    if (existing) return res.status(400).json({ error: '2FA already enabled' });

    const secret = crypto.randomBytes(20).toString('base32');
    const id = uuid();

    const existingSecret = await db.prepare('SELECT * FROM totp_secrets WHERE user_id = ?').get(req.userId!) as any;
    if (existingSecret) {
      await db.prepare('UPDATE totp_secrets SET secret = ?, enabled = 0 WHERE user_id = ?').run(secret, req.userId!);
    } else {
      await db.prepare('INSERT INTO totp_secrets (id, user_id, secret) VALUES (?, ?, ?)').run(id, req.userId, secret);
    }

    const user = await db.prepare('SELECT email FROM users WHERE id = ?').get(req.userId!) as any;
...
router.post('/2fa/confirm', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const { code } = req.body;
    if (!code) return res.status(400).json({ error: 'TOTP code required' });

    const totpRecord = await db.prepare('SELECT * FROM totp_secrets WHERE user_id = ? AND enabled = 0').get(req.userId!) as any;
    ...
    await db.prepare('UPDATE totp_secrets SET enabled = 1 WHERE id = ?').run(totpRecord.id);

    const codes: string[] = [];
    for (let i = 0; i < 8; i++) {
      const recoveryCode = crypto.randomBytes(4).toString('hex').toUpperCase();
      codes.push(recoveryCode);
      await db.prepare('INSERT INTO recovery_codes (id, user_id, code) VALUES (?, ?, ?)').run(
    ...
    const user2fa = await db.prepare('SELECT email FROM users WHERE id = ?').get(req.userId!) as any;
    if (user2fa?.email) {
...
router.post('/2fa/disable', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    const totpRecord = await db.prepare('SELECT * FROM totp_secrets WHERE user_id = ? AND enabled = 1').get(req.userId!) as any;
    ...
    await db.prepare('DELETE FROM totp_secrets WHERE user_id = ?').run(req.userId);
    await db.prepare('DELETE FROM recovery_codes WHERE user_id = ?').run(req.userId);
...
router.post('/2fa/backup-codes', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const totpRecord = await db.prepare('SELECT * FROM totp_secrets WHERE user_id = ? AND enabled = 1').get(req.userId!) as any;
    if (!totpRecord) return res.status(400).json({ error: '2FA not enabled' });
...
router.post('/export', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const user = await db.prepare('SELECT id, email, created_at FROM users WHERE id = ?').get(req.userId!) as any;
    ...
    const links = await db.prepare('SELECT * FROM links WHERE profile_id = ?').all(profile?.id || '');
    const socials = await db.prepare('SELECT * FROM social_links WHERE profile_id = ?').all(profile?.id || '');
    ...
    const loginHistory = await db.prepare('SELECT ip, user_agent, success, created_at FROM login_history WHERE user_id = ? ORDER BY created_at DESC LIMIT 100').all(req.userId!);
    const sessions = await db.prepare('SELECT ip, user_agent, is_active, created_at, expires_at FROM sessions WHERE user_id = ?').all(req.userId!);
...
router.put('/notifications/settings', authMiddleware, async (req: AuthRequest, res) => {
  try {
    const key = `notification_prefs_${req.userId}`;
    const value = JSON.stringify(req.body);
    const existing = await db.prepare('SELECT key FROM system_settings WHERE key = ?').get(key) as any;
    if (existing) {
      await db.prepare("UPDATE system_settings SET value = ?, updated_at = datetime('now') WHERE key = ?").run(value, key);
    } else {
...
router.post('/deactivate', authMiddleware, async (req: AuthRequest, res) => {
  try {
    await db.prepare('UPDATE profiles SET is_deactivated = 1 WHERE user_id = ?').run(req.userId!);
    await db.prepare('UPDATE sessions SET is_active = 0 WHERE user_id = ?').run(req.userId!);
    const ip = String(req.headers['x-forwarded-for'] || req.socket.remoteAddress || '').split(',')[0].trim();
    await db.prepare('INSERT INTO audit_logs (id, user_id, action, target_type, target_id, details, ip) VALUES (?, ?, ?, ?, ?, ?, ?)').run(
